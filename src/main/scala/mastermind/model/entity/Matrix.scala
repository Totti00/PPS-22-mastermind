...
trait Matrix[T]:
  ...
  def cols: Int = if (elements.nonEmpty) elements.head.size else 0
...
object Matrix:
  ...
  private case class MatrixImpl[T](override val elements: Vector[Vector[T]]) extends Matrix[T]:
    ...
    override def replaceCell(row: Int, col: Int, cell: T): Matrix[T] = copy(elements.updated(row, elements(row).updated(col, cell)))